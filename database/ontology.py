...
class ontology:
    ...
    def add_edge(self, type, start, end, time, weight):
        ...
        elif type == "is_a"    and st == et    : valid = True
    ...
    def add_core_objects(self):
        ...
        _type = self.add_node("type", "***core-node***")
        verb = self.add_node("verb", "***core-node***")
        change = self.add_node("change", "***core-node***")
        constraint = self.add_node("constraint", "***core-node***")

        nodes = [noun, relationship, value, _type, verb, change, constraint]

        for node in nodes:
            self.add_edge("knows_of", noun, node, 1, 100)

        for node in nodes:
            self.add_edge("is_a", node, node, 1, 100)

        for node in nodes:
            self.add_edge("describes", relationship, node, 1, 100)

        self.add_edge("has_value", relationship, value, 1, 100)
        self.add_edge("has_type", relationship, _type, 1, 100)
        self.add_edge("regarding", relationship, noun, 1, 100)

        self.add_edge("is_constrained_by", relationship, constraint, 1, 100)
