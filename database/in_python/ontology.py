...
class ontology:
        ...
        if valid:
            ...
            for edge_off_start in self.graph.edges([start]): #itterating bad. Fix, fix, fix
                if edge_off_start[0] == start and edge_off_start[1] == end:
                    new_edge = self.get_edge_val(edge_off_start)
                    is_new_edge = False
                    break
            if is_new_edge:
    ...
    def show_locally(self):
        ...
        for e in self.graph.edges_iter():
            print str(e[0].id) + " -" + str(self.get_edge_val(e).weights) + "-> " + str(e[1].id)
    ...
    def get_edge_val(self, edge_tuple):
        return self.graph[edge_tuple[0]][edge_tuple[1]]["edge"]
...
is on: (bed, chair, table, floor)
Title:
(Dreams of Potatoes; Tequila Sunrise; The Kraken; 40 Cakes; Spectral Robot Task Force; The Vengeful Penguin; Ninja’s Guide to Ornamental Horticulture; Neko-nomicon; This is Not a Book)
