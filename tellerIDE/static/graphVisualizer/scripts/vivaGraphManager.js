	...
	init: function (graphics, graph, node, obj) {
		...
		changeEdgeLength = function (nodeId, size) {
			...
			graph.forEachLinkedNode(nodeId, function (node, link) {
				...
				nodeChanges.push([link, link.fromId, link.toId, {
					lengthRatio: size
				}]);
			});
...
var vivaGraphManager = {
	...
	getRenderer: function (graph, layout, graphics, container) {
	...
	init: function (container, nodes, edges) {
		...
		var renderer = vivaGraphManager.getRenderer(graph, vivaGraphManager.myLayout, graphics, container);
	...
	addAll: function (graph, nodes, edges) {
		...
		for (var n in edges) {
			try {
				vivaGraphManager.helperAddLink(graph, edges[n].source, edges[n].target);
			} catch (exception) {}
		}
	}
